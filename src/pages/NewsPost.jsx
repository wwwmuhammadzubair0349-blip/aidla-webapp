...
export default function NewsPost() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        // SEO meta (after we have title/excerpt)
        const title = data?.title ? `${data.title} | AIDLA News` : "AIDLA News";
        const desc = (data?.excerpt || "Latest AIDLA updates and announcements.").slice(0, 160);
        document.title = title;
        setMetaTag("name", "description", desc);
        setMetaTag("property", "og:title", title);
        setMetaTag("property", "og:description", desc);
        setMetaTag("property", "og:type", "article");
        if (data?.cover_url) setMetaTag("property", "og:image", data.cover_url);
      } catch (e) {
        setErr(e?.message || "News not found.");
        document.title = "AIDLA News";
      } finally {
        setLoading(false);
      }
    };
  ...
  const styles = {
    ...
    featured: { fontSize: 11, fontWeight: 950, color: "#166534", background: "#dcfce7", border: "1px solid #bbf7d0", padding: "5px 10px", borderRadius: 999 },
  ...
  if (err) {
    return (
      <div style={styles.shell}>
        ...
        <main style={styles.main}>
          <div style={styles.inner}>
            <div style={styles.alert("error")}>{err}</div>
            <button style={styles.smallBtn} onClick={() => nav("/news")} type="button">
              ← Back to News
            </button>
          </div>
        </main>
            ...
            <div style={styles.navDesktop}>
              {navItems.map((item) => (
                <Link
                  ...
                  style={{
                    ...styles.navLink,
                    color: item.to === "/news" ? "#4f46e5" : styles.navLink.color,
                  }}
                >
                  {item.label}
                </Link>
        ...
        <div style={styles.inner}>
          ...
          <article style={styles.card}>
            <div style={styles.cover}>
              {row.cover_url ? <img src={row.cover_url} alt={row.title} style={styles.coverImg} /> : null}
            </div>

            <div style={styles.content}>
              <h1 style={styles.title}>{row.title}</h1>

              <div style={styles.meta}>
                <span>{fmtDateTime(row.published_at || row.created_at)}</span>
                ...
                {row.source_url ? (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    style={{ color: "#1d4ed8", fontWeight: 950, textDecoration: "none" }}
                  >
                    Source ↗
                  </a>
                ) : null}
              ...
              <div style={styles.pills}>
                {row.is_featured ? <span style={styles.featured}>Featured</span> : null}
                {(row.tags || []).slice(0, 6).map((t) => (
                  <span key={t} style={styles.pill}>{t}</span>
                ))}
              </div>

              {row.excerpt ? <div style={styles.excerpt}>{row.excerpt}</div> : null}

              <div style={styles.body}>{row.content || ""}</div>
            </div>
          </article>
        </div>
      </main>
